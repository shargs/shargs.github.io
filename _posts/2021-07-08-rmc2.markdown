...
In this post, we'll discuss the background behind Gabidulin codes, which are an important family of rank metric codes, and design a simple encoder. Throughout this post, $$q$$ is a prime power.
...
In other terms, we evaluate $$\widehat P$$ instead of $$P$$. In the rank metric, Gabidulin codes attain the Delsarte-Singleton bound (see Part I) and are therefore MRD. 
...
$$b_1 = \zeta, b_2 = \zeta^{[2]}, b_3 = \zeta^{[3]}, \dotsc, b_m = \zeta^{[m]}$$
...
$$g_i^{[j]} = b_i^{[j]} = \zeta^{[i][j]} = \zeta^{[i+j]} = \zeta^{[i+j ~ \bmod ~ m]}.$$
...
```python
G = Matrix([[zeta^(q^(i+j)) for j in range(k)] for i in range(n)])
```
...
def as_bytes(msg):
    ...
    msg_bits = [Fq(ZZ(a).digits(2)) for a in str.encode(msg)]
```
...
There we have it, in a total of 25 SageMath lines (comments, whitespace, and pretty-printing included!) the promised Gabidulin encoder. With minor modifications, the code should work with various choices of parameters and help you get a feeling of the encoding process. We also encourage interested readers to implement an encoder at a lower level, say in Rust or C, where our choices of parameters allow for clever improvements not discussed here.
...
One may wonder why we didn't "just give the generator matrix" right away. Besides the fact that one needs to build this matrix differently for different situations, and the various properties that require some theory to understand, there is really no other way to see that Gabidulin codes are to the rank metric what Reed-Solomon codes are to the Hamming metric. 

But the real, deep reason for this burst of theory is something we'll see in the **next part** (to be written). As is typical in code theory, decoding is the hard part. Erros in the rank metric are also strange beasts, we'll look into that. Stay tuned!
